...
export const TerminalDrawer: React.FC<TerminalDrawerProps> = ({ logs }) => {
  ...
  return (
    <div className="rounded-2xl border border-neutral-200/80 bg-white overflow-hidden shadow-card text-xs">
      {/* Header Toggle */}
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full flex items-center justify-between px-5 py-3.5 bg-neutral-50/70 hover:bg-neutral-100/80 text-neutral-700 transition-colors border-b border-neutral-200/60"
      >
        <div className="flex items-center gap-2.5">
          <Terminal className="h-4 w-4 text-emerald-600" />
          <span className="font-semibold text-xs text-neutral-900 tracking-tight">
            Terminal CloudBaik VPS & Hermes Agent Telemetry
          </span>
          <span className="text-[10px] font-mono bg-emerald-100/70 text-emerald-800 border border-emerald-300/60 px-2 py-0.5 rounded-full">
            RAM: 142MB / 4GB
          </span>
        </div>
      ...
      {/* Terminal Body */}
      {isOpen && (
        <div className="p-4 max-h-60 overflow-y-auto space-y-1.5 bg-[#0A0E17] text-neutral-300 font-mono text-xs select-text">
          ...
          {logs.map((log, idx) => {
            const isAlert = log.includes('BAHAYA') || log.includes('CRASH') || log.includes('Defisit');
