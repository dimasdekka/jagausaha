...
>(function ExpandingArrowButton(
  ...
  return (
    <motion.button
      ...
      className={cn(
        "relative inline-flex h-14 min-w-64 items-center overflow-hidden rounded-full bg-neutral-950 p-1.5 text-white select-none cursor-pointer",
        "outline-none focus-visible:ring-2 focus-visible:ring-neutral-400 focus-visible:ring-offset-2",
    ...
    >
      <motion.span
        ...
        style={{
          width: active ? "calc(100% - 12px)" : 44,
          borderRadius: 9999,
        }}
        className={cn(
          "absolute inset-y-1.5 left-1.5 z-10 overflow-hidden bg-white text-neutral-950",
          accentClassName,
        )}
      >
        <motion.span
          ...
          className="absolute inset-0 grid place-items-center text-neutral-950"
        >
          <DottedChevron className="h-5 w-4" />
        </motion.span>

        <span className="absolute inset-0 flex items-center justify-around px-3 text-neutral-950">
      ...
      <motion.span
        ...
        className={cn(
          "relative z-0 ml-[60px] mr-5 whitespace-nowrap text-sm font-semibold tracking-tight",
          labelClassName,
        )}
      >
        {children}
