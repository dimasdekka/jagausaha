"use client";
// beui.dev/components/motion/text-animation (TextReveal)

import { motion, type Transition, useInView, useReducedMotion } from "framer-motion";
  ...
  return (
    <Comp ref={ref} className={cn("block", className)}>
      {lines.map((line) => {
        ...
        const renderUnit = (unit: string) => {
          ...
          const transition: Transition = reduce
            ...
            : {
                ...
                opacity: { duration: 0.6, ease: EASE_OUT, delay: d },
                filter: { duration: 0.8, ease: EASE_OUT, delay: d },
              };
          return (
            <motion.span
              key={unitKey}
        ...
        return (
          <span key={lineKey} className="inline">
            {groups.map((group) => {
